...
class TBGFlowNetGenerator(nn.Module):
    ...
    def get_loss(self, batch):
        ...
        # print("logZ: ", self.model.Z.log())
        # print("seq_logit_norm: ", seq_logits_norm)
        # print("log_reward: ", r.clamp(min=self.reward_exp_min).log())
        loss = (self.model.Z.log() + seq_logits_norm - r.clamp(min=self.reward_exp_min).log()).pow(2).mean()
...
class FMGFlowNetGenerator(nn.Module):    
    def __init__(self, args, tokenizer):
        super().__init__()
        self.args = args
        self.leaf_coef = args.gen_leaf_coef
    ...
    def preprocess_state(self, input_batch):
        s = self.tokenizer.pad_tokens(sum(input_batch["traj_states"], [])).to(self.device)
        inp_x = F.one_hot(s, num_classes=self.num_tokens+1)[:, :, :-1].to(torch.float32)
        inp = torch.zeros(s.shape[0], self.max_len, self.num_tokens)
        inp[:, :inp_x.shape[1], :] = inp_x
